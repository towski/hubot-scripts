...
nowPlayingForUser = (user, msg) ->
  if user.lastfm?.username?
    nowPlayingForLastfmUser user.lastfm.username,
      success: (track) ->
        if track
          msg.send colors.red("xX-" + user.name + "-Xx") + " is listening to " + colors.blue.bold(track.title) + " by " + colors.purple.bold(track.artist)
        else
          msg.send "#{user.name} is not scrobbling anything!"
      error: (error) ->
        msg.send "Oh no! An error occurred: #{error.message}"
  else
    msg.send "I don't know #{user.name}'s Last.fm username! Use 'slaybert I'm <username> on last.fm'"
